...
class EmailService {
  ...
  constructor() {
    this.supportEmail = config.email.supportEmail
  ...
  async sendEscalationEmail(payload: EscalationEmailPayload) {
...
<body>
  <div class="container">
    ...
    const mailOptions = {
      from: `"SmartSupport AI" <${config.email.smtpUser || 'no-reply@smartsupport.ai'}>`,
    ...
    try {
      ...
      console.log(`✅ Escalation email sent. Message ID: ${(info as any).messageId}`)
