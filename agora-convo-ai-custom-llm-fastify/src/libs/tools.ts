...
type FunctionHandler = (appId: string, userId: string, channel: string, args: Record<string, any>) => Promise<string>
...
async function resolveCustomerId(customerId?: string, customerEmail?: string): Promise<string | null> {
  if (customerId) {
    return customerId
  }
  if (!customerEmail) {
...
async function createSupportTicket(
  ...
  if (!resolvedEmail && resolvedCustomerId) {
    const { data: customer } = await supabase
      ...
      .maybeSingle()
    resolvedEmail = (customer?.data as Record<string, any>)?.email ?? null
  }
  ...
  const { data, error } = await supabase
    .from(config.support.ticketsTable)
    .insert(payload)
    .select()
    .single()
...
async function logCustomerInteraction(
  ...
  const { data, error } = await supabase
    .from(config.support.interactionsTable)
    .insert(payload)
    .select()
    .single()
...
async function getTicketDetails(
  ...
  let interactionEntries: Array<Record<string, any>> | undefined
...
export type { PeerMessageResponse, FunctionHandler }
